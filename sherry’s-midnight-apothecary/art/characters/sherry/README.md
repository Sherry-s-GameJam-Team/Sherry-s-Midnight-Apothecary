...
运行 `res://art/characters/sherry/sherry_test_scene.tscn` 可打开固定镜头的主角动作测试。使用左右方向键移动，按住 Shift 跑步。舞台边界固定在画面内，便于直接检查角色的比例、脚底对齐和动作衔接。
...
`sherry_test_scene.gd` 只处理移动、状态切换和边界；它不引用任何具体图片。所有帧范围、播放速度与循环规则统一维护在 `sherry_animation_library.gd` 的 `ACTIONS`。测试场景还可按 Space 预览投掷、H 预览受击、F 预览倒地。
