class_name MapSwitchStation
extends Area2D

## Home-scene entry point for the shared Magic Map Switch modal.
## The map UI lives under AppRoot/GlobalUI, so opening this station never changes
## the current level scene.

@export_node_path("Sprite2D") var visual_path: NodePath
@export var interaction_hint_text := "按 [E] 使用魔法地图"
...
func _ready() -> void:
	...
	if _visual == null:
		push_error("MapSwitchStation requires a Sprite2D visual.")
		return
	monitoring = true
...
func _input(event: InputEvent) -> void:
	if get_tree().has_meta("day_modal_input_locked") or not _player_inside or not _is_interact_event(event):
		return
	var map_switch := _find_map_switch()
	if map_switch == null or map_switch.visible:
		return
...
func _find_map_switch() -> Control:
	var current: Node = self
	while current != null:
		var interaction := current.get_node_or_null("GlobalUI/MapSwitchInteraction") as Control
		if interaction != null:
			return interaction
