...
`day_one_forest_enzuo_rescued.tres` 定义 `day_one_forest_enzuo_saved` 的完成标记契约。森林路线属于第 1 天：树冠 Boss 未净化时显示 `issue_save_enzuo`，但不播放恩佐对话；Boss 完成后该节点隐藏并显示 `issue_saved/Enzuofall`，自动派发该事件。对话完成会写入 `save_enzuo_solved` 与完成标记，并在本地黑场后调用 `DayRuntime.finish_day()` 进入夜晚经营。它不授予物品，也不改变水车或树门进度。
