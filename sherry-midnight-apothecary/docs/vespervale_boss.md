...
- Boss 血量归零后：
  1. 播放战败动画，停止梦潮循环并即刻清除场地上的所有残留弹幕与召唤阵。
  2. 自动锁定角色控制，弹出官方通关结算 UI（`TaskCompleteUI`），展示金色羽饰弹窗、完成音效（`CompleteChime` / `SparkleChime`）与净化文本：
     - **标题**：“维斯佩尔梦疗院已被净化”
     - **描述**：“院长与沉睡的患者已从梦魇中解脱，暮息庭院重现生机。”
     - **页脚提示**：“点击继续将返回暮息庭院”
  3. 玩家点击继续后触发**全屏黑屏渐隐淡出**，平滑转场并将玩家传送回暮息庭院静语礼堂前庭（`vespervale_garden` 的 `church` 出生点）。
  4. 回到庭院后，沉睡 NPC 全部隐退，左侧金色传送门激活开启，可按 `E` 结束今日探索并开启晚间营业。
- 从回廊 (`inner.tscn`) 进入方式：
  - 玩家沿长廊向右行进至最右侧边界即可直接无缝触碰传送到深层花园 Boss 场景。
