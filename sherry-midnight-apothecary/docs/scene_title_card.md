...
The daytime `SceneTitleCard` displays the day number (including the game's opening day 0), the current level's `display_name`, the level's `disaster_name` directly below the location, and one subtitle line below that.
...
Every daytime environment inherits `set_corrupted(corrupted)` and `is_corrupted()` from `DayLevelEnvironment`. Levels can override `set_corrupted()` to switch visuals; Grassland does so for its sky, horizon, and grass textures. `disaster_days` remains available for legacy day-based title data.
