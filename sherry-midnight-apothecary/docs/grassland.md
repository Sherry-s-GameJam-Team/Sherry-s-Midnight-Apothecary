...
After Luca's purification tutorial records `sleeping_hound_purification_complete`, the Grassland HomeDoor no longer travels to the daytime Home scene. It presents a confirmation warning — “进入药水铺将开启晚间营业，请确保资源收集完毕” — and confirmation emits a completed `DayResult` through `DayRuntime`, which moves `GameFlow` into `NightRuntime`. Before that Luca-task flag is set, including after the Emerald Field miasma purification alone, the door keeps its ordinary daytime Home destination. Cancelling keeps the player in Grassland to continue collecting resources.
