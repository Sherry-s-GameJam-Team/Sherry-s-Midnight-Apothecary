...
遗址对话结束后，流程设置 `day_six_crownland_escort_pending` 并切换到 `crownland` 的 `from_home` 入口。`DaySixCrownlandEscort` 接管输入，在 `crownland.tscn` 中显示真实的雪莉 Player、恩佐 `AnimatedSprite2D` 与 Luca 场景实例，并让三人一边行进一边自动对话。三人的脚底分别按自身碰撞或贴图原点校准至 `WorldBounds/Ground` 顶面，移动补间只改变 X 轴，因此队伍沿地面水平推进至 `EntryPoints/cathedral` 的 X 坐标；即使玩家快速推进对话，大教堂事件也会将三人准确吸附到同一地面基线并切换待机动画。
