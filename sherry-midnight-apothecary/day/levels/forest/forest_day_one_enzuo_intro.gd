...
func _play_dialogue(title: StringName) -> void:
	await _play_dialogue_resource(dialogue_resource, title)


func _play_dialogue_resource(resource: DialogueResource, title: StringName) -> void:
	if resource == null:
		return
	...
	if dialogue_manager == null or not dialogue_manager.has_method("show_dialogue_balloon_scene"):
		...
		return
	var balloon := dialogue_manager.show_dialogue_balloon_scene(BALLOON_SCENE, resource, title) as Node
	if balloon != null:
