class_name VespervaleLucaFollow
extends Node

## Scene-local companion movement based on SewerLucaFollow. Luca keeps a
## hysteresis gap behind Sherry and accelerates only when substantially behind.
...
func _ready() -> void:
	...
	if _luca == null:
		push_warning("VespervaleLucaFollow requires a LucaPlayer at luca_path.")
