...
func _resolve_dialog() -> void:
	if dialog == null:
		...
		if root != null:
			dialog = root.find_child("DialLockPuzzleDialog", true, false) as DialLockPuzzleDialog
	if dialog != null:
		if not dialog.lock_unlocked.is_connected(_on_lock_unlocked):
			dialog.lock_unlocked.connect(_on_lock_unlocked)
		if not dialog.dialog_closed.is_connected(_on_dialog_closed):
