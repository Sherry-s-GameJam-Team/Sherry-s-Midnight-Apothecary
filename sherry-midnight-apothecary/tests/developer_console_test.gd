...
static func run(test: TestSupport) -> void:
	...
	test.expect(day_console.execute_command("scene list").contains("[19] aurem_clockyard"), "Console lists scene 19 aurem_clockyard.")
	test.expect(day_console.execute_command("scene list").contains("[20] aurem_clockyard_inside"), "Console lists sub-scene 20 aurem_clockyard_inside.")
	test.expect(day_console.execute_command("scene list").contains("[26] control_system_demo"), "Console lists sub-scene 26 control_system_demo.")
	test.expect_equal(day_console.execute_command("scene 1"), "scene = 流明街", "Console opens scene 1 (Town/Market) by number.")
	test.expect_equal(day_console.execute_command("scene 19"), "scene = 奥勒姆钟庭", "Console opens scene 19 (Aurem Clockyard) by number.")
	test.expect_equal(day_console.execute_command("scene 20"), "scene = 奥勒姆巨钟塔·内部", "Console opens sub-scene 20 (Clockyard Inside) by number.")
