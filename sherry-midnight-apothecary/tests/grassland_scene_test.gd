...
static func run(test: TestSupport) -> void:
	...
	var completed_entry := grass.get_node("EntryPoints/level_completed") as Marker2D
	...
	test.expect(completed_entry != null, "Grassland exposes the purifier-completion return entry point.")
	...
	test.expect_equal(camera.limit_left, -1383, "Grassland camera follows the current left world barrier.")
	test.expect_equal(camera.limit_right, 3407, "Grassland camera follows the current right world barrier.")
