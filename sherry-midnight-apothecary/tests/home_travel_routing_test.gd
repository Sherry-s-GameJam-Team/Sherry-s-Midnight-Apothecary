...
static func run(test: TestSupport) -> void:
	...
	test.expect(not map_controller.can_lock_destination({"id": &"forest"}, locked_player_data), "Future locked anchors cannot be confirmed.")
	locked_player_data.unlock_level(&"forest")
	test.expect(map_controller.can_lock_destination({"id": &"forest"}, locked_player_data), "An activated future anchor becomes confirmable.")
	map_controller.free()
	...
	test.expect(runtime.activate_travel_anchor(&"forest"), "DayRuntime activates the next travel anchor.")
	test.expect(runtime.set_home_destination(&"forest"), "Home can lock an anchor after it is activated.")
	test.expect_equal(player_data.active_home_destination_id, &"forest", "The selected Home destination is stored in PlayerData.")
	runtime.free()
